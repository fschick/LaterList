...
namespace FS.LaterList.Repository.SQLite.Repositories
{
    public class LaterListRepository : ILaterListRepository
    {
        ...
        public TEntity Remove<TEntity>(TEntity entity) where TEntity : class, IModel
        {
            var result = _dbContext.Remove(entity).Entity;
            _dbContext.SaveChanges();
            return result;
        }

        public List<TEntity> RemoveRange<TEntity>(List<TEntity> entities) where TEntity : class, IModel
        {
            var result = entities
